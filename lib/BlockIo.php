...
class BlockIo
{
    ...
    { // method_missing for PHP
        ...
        if (in_array($name,
            $this->withdrawal_methods)) { // it is a withdrawal method, let's do the client side signing bit
    ...
    {
        // Generate cURL URL
        $url = str_replace("API_CALL", $path, "https://block.io/api/v".$this->version."/API_CALL/?api_key=")
               .$this->api_key;
        $addedData = "";
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            ...
            if (!file_exists($pemfile)) {
                throw new \Exception("Needed .pem file not found. Please download the .pem file at http://curl.haxx.se/ca/cacert.pem and save it as "
                                    .$pemfile);
            }
    ...
    { // withdraw method to be called by __call
        ...
        if ($response->status == 'success'
            && array_key_exists('reference_id', $response->data)) { // we have signatures to append
